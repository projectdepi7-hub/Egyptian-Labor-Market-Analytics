...
select
  ...
  ,COUNT(e.`satisfied_current_job`) as `# employees`
from employee e
inner join dim_occupation occ on e.occupation_key = occ.occupation_key
...
select
  occ.occupation_one_digit as `Occupation Category`
  ,e.earnings_wages as `satisfaction`
  ,COUNT(e.`earnings_wages`) as `# employees`
from employee e
inner join dim_occupation occ on e.occupation_key = occ.occupation_key
...
select
  ...
  ,COUNT(e.`job_stability`) as `# employees`
from employee e
inner join dim_occupation occ on e.occupation_key = occ.occupation_key
...
select * from dim_individual;
select * from employee;

Select d.ind_key, e.ind_key, e.individual_key
from dim_individual As d, employee as e
WHERE e.individual_key = 1000 AND d.ind_key = e.ind_key;


select count(ind_key) from dim_individual;
select count(ind_key) from employee;
select count(individual_key) from employee;


select ind_key from dim_individual
where ind_key not in (
select distinct(individual_key) from employee
);

# 14- Health Insurance according to regions
SELECT
  rgn.region_group AS `Region`,
  e.have_health_insurance AS `Health Insurance`,
  COUNT(*) AS `# employees`
FROM employee e
INNER JOIN dim_region rgn ON e.region_key = rgn.region_key
GROUP BY rgn.region_group, e.have_health_insurance
ORDER BY `Region` DESC;


# 15- No Health Insurance according to regions
SELECT
  rgn.region_group AS `Region`,
  e.have_health_insurance AS `Health Insurance`,
  COUNT(*) AS `# employees`
FROM employee e
INNER JOIN dim_region rgn ON e.region_key = rgn.region_key
where e.have_health_insurance in ("No health insurance")
GROUP BY rgn.region_group, e.have_health_insurance
ORDER BY `Region` DESC;

###### Count of employees according to regions
SELECT
  rgn.region_group AS `Region`,
  COUNT(e.region_key) AS `# employees`
FROM employee e
INNER JOIN dim_region rgn ON e.region_key = rgn.region_key
GROUP BY rgn.region_group
ORDER BY `Region` DESC;


# 16- Social Insurance according to regions
SELECT
  rgn.region_group AS `Region`,
  e.social_insurance AS `Social Insurance`,
  COUNT(*) AS `# employees`
FROM employee e
INNER JOIN dim_region rgn ON e.region_key = rgn.region_key
GROUP BY rgn.region_group, e.social_insurance
ORDER BY `Region` DESC;

# 17- AVG Wages per occupation according to education
SELECT 
 occ.occupation_one_digit as `Occupation Category`,
 ed.`educational_attainment` as `EDUCATION`,
 COUNT(*) AS Num_People,
ROUND(AVG(e.monthly_wage), 0) AS Avg_Wage
FROM employee e
JOIN dim_edu ed ON e.edu_key = ed.edu_key
JOIN dim_occupation occ ON e.occupation_key = occ.occupation_key
GROUP BY occ.occupation_one_digit, ed.educational_attainment
ORDER BY occ.occupation_one_digit, Avg_Wage;